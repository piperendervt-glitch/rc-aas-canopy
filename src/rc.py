...
# R2実験：もったいない精神（重みの再配置）
WEIGHT_REVIVE_THRESHOLD = 0.05   # 死亡寸前の閾値
WEIGHT_TRANSFER_AMOUNT = 0.05    # 移動するweight量
...
class RC:
    ...
    def monitor(self, weights: dict, accuracy: dict, arm_weights: dict = None) -> list:
        ...
        for arm_id, weight in weights.items():
            ...
            if weight < WARNING_THRESHOLD:
                ...
                alerts.append({
                    ...
                    "arm": arm_id,
                    "weight": weight,
                    "message": f"[RC] 腕{arm_id} weight={weight:.3f} → 上限閾値を超えた（過集中）"
                })
                # 過集中も急性カウンタで追跡する
                self.cutoff_counters[arm_id] = self.cutoff_counters.get(arm_id, 0) + 1
                self.warning_accum_counters[arm_id] = self.warning_accum_counters.get(arm_id, 0) + 1
                ...
                if self.cutoff_counters[arm_id] >= CUTOFF_COUNT:
                    ...
                    alerts.append({
                        ...
                        "message": (
                            f"[RC] 腕{arm_id} weight={weight:.3f}"
                            f" (過集中連続{self.cutoff_counters[arm_id]}回"
                            f" / pending{self.cutoff_pending_counters[arm_id]}回)"
                            f" → 切断候補。人間の判断を要請。"
                        )
                    })

                    # ステージ1：セッション内5回 → 封印レベル1
                    if self.cutoff_pending_counters[arm_id] >= TIMEOUT_STAGE1:
                        self._auto_seal_level1(arm_id)
                        alerts.append({
                            "level": "auto_seal_1",
                            ...
                            "pending_count": self.cutoff_pending_counters[arm_id],
                            "message": (
                                f"[RC] 腕{arm_id} cutoff_pending × {self.cutoff_pending_counters[arm_id]}回（過集中）"
                                f" → 封印レベル1（flow_weight凍結）に自動移行。"
                                f" 人間の確認を要請。"
                            )
                        })
        ...
        # R2：もったいない精神（死亡寸前パスの復活）
        revive_alerts = self.revive_dying_paths(weights)
        alerts.extend(revive_alerts)
    ...
    def revive_dying_paths(self, weights: dict) -> list:
        """
        死亡寸前のパスを検知して隣接パスからweightを移す。

        ルール：
          1. flow_weight < WEIGHT_REVIVE_THRESHOLD のパスを「死亡寸前」と判定
          2. 隣接パスから WEIGHT_TRANSFER_AMOUNT だけ移す
          3. 移し元のパスが0.1以上ある場合のみ移す
          4. flow_weightの合計は変わらない（移動のみ）
          5. 新しいパスは作らない
          6. 人間に通知する
          7. 封印済みパスは対象外
        """
        ...
        for path in paths:
            if path in self.sealed_paths:
                continue
            if weights[path] < WEIGHT_REVIVE_THRESHOLD:
                donor = next(
                    (p for p in paths if p != path
                     and p not in self.sealed_paths
                     and weights[p] >= 0.1),
                    None
                )
                if donor:
                    old_val = weights[path]
                    weights[path] += WEIGHT_TRANSFER_AMOUNT
                    weights[donor] -= WEIGHT_TRANSFER_AMOUNT
                    msg = (f"[REVIVE] パス{path}: {old_val:.3f}->{weights[path]:.3f}"
                           f" ({donor}から{WEIGHT_TRANSFER_AMOUNT}移転)")
                    alerts.append({
                        "level": "revive",
                        "arm": path,
                        "donor": donor,
                        "old_weight": old_val,
                        "new_weight": weights[path],
                        "message": msg,
                    })
