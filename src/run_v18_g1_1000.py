"""
run_v18_g1_1000.py
v18実験（③ G1拡張）：1000問プールでベスト初期値を検証

v16ベスト [0.3,0.7,0.7] + ベースライン [0.5,0.5,0.5] を
generate_tasks(count=1000) で比較
"""
...
PATTERNS = [
    {"id": "baseline_0.5", "name": "均一[0.5,0.5,0.5]", "weights": make_weights(0.5, 0.5, 0.5)},
    {"id": "best_0.3_0.7_0.7", "name": "v16ベスト[0.3,0.7,0.7]", "weights": make_weights(0.3, 0.7, 0.7)},
    {"id": "second_0.7_0.7_0.3", "name": "v16次点[0.7,0.7,0.3]", "weights": make_weights(0.7, 0.7, 0.3)},
]
...
def main():
    print("=== v18実験（G1拡張）：1000問プール検証 ===")
    print()

    tasks = generate_tasks(count=1000)
    print(f"タスク数: {len(tasks)}（1000問プール）")
    print()

    output_path = RESULTS_DIR / "v18_g1_1000.jsonl"
    all_results = []
    ...
    print("=" * 60)
    print("v18実験 結果（1000問プール）")
    print("=" * 60)
    for r in all_results:
        print(f"  {r['pattern_id']:<25} acc={r['accuracy']:.1%}  warnings={r['rc_warnings']}")

    print(f"\n100問との比較：")
    print(f"  v14 均一(100問): 50.0%  →  v18 均一(1000問): {all_results[0]['accuracy']:.1%}")
    print(f"  v16 ベスト(100問): 62.0%  →  v18 ベスト(1000問): {all_results[1]['accuracy']:.1%}")
