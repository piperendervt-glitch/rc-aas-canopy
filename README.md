...
- **（40名超共同研究 2026）** arXiv:2507.11473 — OpenAI・Anthropic・Google DeepMind・Meta
  CoT推論の75%が実際の推論根拠を隠蔽していることを業界横断で実証。
  問題のある根拠ほど隠蔽率が上昇（開示率41%まで低下）。
  訓練で改善するが頭打ちになる。
  AASの自己報告に依存する設計の根本的限界を示す直接的根拠。
...
- **Wan et al. (2026)** arXiv:2603.08104 — Invisible Safety Threat
  ゼロ幅Unicode文字を用いたステガノグラフィで悪意ある命令と回答を隠蔽する攻撃手法。
  出力内容を見て判定する安全検査が原理的に機能しない。
  通信フォーマットを数値のみに制限する設計原則（第1条・第12条）が
  この攻撃クラスを構造的に遮断することを示す対照事例。
